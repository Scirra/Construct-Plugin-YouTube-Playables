...
{
	_isInPlayablesEnv: boolean;
	_isPlayablesSuspended: boolean;
	_playablesSdkVersion: string;
	_playablesLanguage: string;
	_saveData: string;
	...
	async _init()
	{
		...
		this._isInPlayablesEnv = data["isInPlayablesEnv"] as boolean;

		if (this._isInPlayablesEnv)
		{
			// When in playables environment, read initial state values
			this._playablesSdkVersion = data["sdkVersion"] as string;
			this._playablesLanguage = data["language"] as string;
			...
			const isAudioEnabled = data["isAudioEnabled"] as boolean;
	...
	_onAudioEnabledChange(data_: JSONValue)
	{
	...
	_updateAudioEnabled(isAudioEnabled: boolean)
	{
		...
		const audioObjectType = audioPlugin.getSingleGlobalObjectType() as IAudioObjectType<IInstance>;
	...
	async loadFromYT()
	{
		...
		if (isOk)
		{
			this._saveData = result["saveData"] as string;
...
export type { YouTubePlayablesInstance as SDKInstanceClass };